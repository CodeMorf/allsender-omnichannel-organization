...
export function createOrganizationRoutes({ models, middlewares = {}, validators, resolveWorkspaceId, service = null, allowUnsafeForTests = false } = {}) {
  ...
  router.delete('/departments/:id/members/:userId', usePermission(checkPermission, 'manage.organization'), asyncRoute(async (req, res) => {
    const data = await organization.removeMembership({ workspaceId: workspace(req), departmentId: req.params.id, areaId: req.query.areaId, userId: req.params.userId });
    return res.json({ success: true, data });
  }));
